...
make_target() {
  ...
  for UBOOT_TARGET in $UBOOT_CONFIG; do
    ...
    make CROSS_COMPILE=${TARGET_NAME}- ARCH=$TARGET_ARCH HOSTCC="$HOST_CC" HOSTSTRIP="true" LOGO_BMP="$LOGOIMG"

    if [ $UBOOT_TARGET_CNT -gt 1 ]; then
      [ -f u-boot.img ] && mv u-boot.img u-boot-$UBOOT_TARGET_NAME.img || :
      [ -f u-boot.imx ] && mv u-boot.imx u-boot-$UBOOT_TARGET_NAME.imx || :
      [ -f u-boot-sunxi-with-spl.bin ] && mv u-boot-sunxi-with-spl.bin uboot-sunxi-$UBOOT_TARGET_NAME.bin || :
